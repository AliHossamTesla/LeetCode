...
void doYourJop()
{
	map<int,road>connect ;
	int Q ;
	cin >> n >> Q ;
	for(int i = 0 ; i < n ; i ++){
		int val ; cin >> val ;
		if(connect.count(val) == 0){
			connect[val].start = connect[val].end = i ;
		}else
			connect[val].end = i ;
	}
	while(Q--){
		int u , v ;
		cin >> u >> v ;
		if(connect.count(u) == 0 or connect.count(v) == 0 or connect[u].start > connect[v].end)
			cout << "NO\n" ;
		else
			cout << "YES\n" ;
	}
