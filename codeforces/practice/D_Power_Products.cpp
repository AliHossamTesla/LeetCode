...
vector<pair<int, int>> primeFactorization(ll n)
{
    ...
    {
        if (i.second % m != 0)
            vp.push_back({i.f, i.second % m});
    }
    return vp;
}
vector<pair<int, int>> inversePowers(vector<pair<int, int>> x)
{
    int s = x.size();
    vector<pair<int, int>> vp(s);
    for (int i = 0; i < s; i++)
        vp[i] = {x[i].first, m - x[i].second};
    return vp;
}
void doYourJop()
{
    ...
    {
        cin >> v[i];
        ma[primeFactorization(v[i])]++;
    }
    ll ans = 0;
    for (auto i : v)
    {
        if (primeFactorization(i) == inversePowers(primeFactorization(i)))
            ans += (ma[primeFactorization(i)] - 1);
        else
            ans += ma[inversePowers(primeFactorization(i))];
    }
    cout << ans / 2 << el;
}
int main()
